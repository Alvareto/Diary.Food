...
namespace Diary.Tests
{
    public abstract class DiaryTestBase : AbpIntegratedTestBase<DiaryTestModule>
    {
        ...
        protected void UsingDbContext(Action<DiaryDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DiaryDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DiaryDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DiaryDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DiaryDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DiaryDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DiaryDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DiaryDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DiaryDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DiaryDbContext, Task<T>> func)
        {
            T result;
